function solution1(input) {
    ...
    while (i < instructions.length) {
        if (instructions[i].visited) return acc
        else instructions[i].visited = true
        if (instructions[i].command === 'acc') acc += instructions[i].amount
        if (instructions[i].command === 'jmp') i += instructions[i].amount - 1
        i++
    }
    return -1
}

function findAcc(instructions) {
    let acc = 0
    let i = 0
    while (i < instructions.length) {
        if (instructions[i].visited) return null
        else instructions[i].visited = true
        if (instructions[i].command === 'acc') acc += instructions[i].amount
        if (instructions[i].command === 'jmp') i += instructions[i].amount - 1
        i++
...
function createContexts(instructions) {
    for (let i = 0; i < instructions.length; i++) {
        if (instructions[i].command === 'nop' || instructions[i].command === 'jmp') {
            const ctx = [...instructions.map(tmp => ({...tmp}))]
            if (ctx[i].command === 'nop') ctx[i].command = 'jmp'
            else if (ctx[i].command === 'jmp') ctx[i].command = 'nop'
            const res = findAcc(ctx)
