...
function addAndMult(input, part) {
    input = input.split(' ')
    ...
    for (let i = 0; i < input.length && part === 2; i++) {
        if (input[i] === '*') {
            input[i-1] = `${+input[i-1] * +input[i+1]}`
            input.splice(i, 2)
            i--
...
function calculate(input, part) {
    let lastParenthesis = []
    for (let i = 0; i < input.length; i++) {
        const chr = input[i];
        if (chr === '(') {
            lastParenthesis.push(i)
        }
        if (chr === ')') {
            const start = lastParenthesis.pop()
            const slice = addAndMult(input.slice(start + 1, i), part)
            input = input.substr(0, start) + slice + input.substr(i + 1);
            i = start
        }
    }
    return +addAndMult(input, part)
}

function solution1(input) {
    let score = 0
    input.split('\n').forEach(entry => {
        score += calculate(entry, 1)
    })
    return score
}
...
const input = `(9 * 3 + 4) * (7 + (2 + 3 + 6 * 2) + 3) * 4 * 5
