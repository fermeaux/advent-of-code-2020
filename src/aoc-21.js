...
function solution1(input) {
    ...
    const allergenes = {}
    input.split('\n').forEach(entry => {
        const g = /^(?<ingredients>.+) \(contains (?<allergenes>.+)\)$/.exec(entry).groups
        const allergenesTmp = g.allergenes.split(', ')
        const ingredientsTmp = g.ingredients.split(' ')
        ingredientsTmp.forEach(i => {
            if (!ingredients[i]) ingredients[i] = { count:0, allergenes: {} }
            ingredients[i].count++
            allergenesTmp.forEach(a => {
                if (!allergenes[a]) allergenes[a] = { count:0, ingredients: {} }
                if (!ingredients[i].allergenes[a]) ingredients[i].allergenes[a] = { count:0 }
                if (!allergenes[a].ingredients[i]) allergenes[a].ingredients[i] = { count:0 }
                ingredients[i].allergenes[a].count++
                allergenes[a].ingredients[i].count++
            })
        })
        allergenesTmp.forEach(a => {
            allergenes[a].count++
    ...
    const map = {}
    const allergeneKeys = Object.keys(allergenes)
    while (allergeneKeys.length > 0) {
        for (let i = 0; i < allergeneKeys.length; i++) {
            const aKey = allergeneKeys[i]
            const a = allergenes[aKey]
            let possibilities = Object.keys(a.ingredients).sort((tmp1, tmp2) => a.ingredients[tmp2].count - a.ingredients[tmp1].count)
            possibilities = possibilities.filter(tmp => !ingredientsUsed[tmp])
            const bestOne = a.ingredients[possibilities[0]].count
            possibilities = possibilities.filter(tmp => a.ingredients[tmp].count === bestOne)
            if (possibilities.length === 1) {
                map[aKey] = possibilities[0]
                ingredientsUsed[possibilities[0]] = true
                allergeneKeys.splice(i, 1)
                i--
            }
        }
    }
    return Object.keys(ingredients).reduce((prev, iKey) => {
        if (!ingredientsUsed[iKey]) return prev + ingredients[iKey].count
        return prev
    }, 0)
}
...
function solution2(input) {
    ...
    return Object.keys(map).sort().map(tmp => map[tmp]).join(',')
}
