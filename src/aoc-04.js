function solution1(input) {
    ...
    entries.forEach(entry => {
        if (entry.length > 0) {
            const infos = entry.split(' ')
            infos.forEach(info => {
                const infoSplitted = info.split(':')
                currentPass[infoSplitted[0]] = infoSplitted[1]
            })
        } else {
            if (currentPass.byr && currentPass.iyr && currentPass.eyr && currentPass.hgt && currentPass.hcl && currentPass.ecl && currentPass.pid) {
                count++
            }
            currentPass = {}
        }
...
function solution2(input) {
    ...
    let currentPass = {}
    let count = 0
    const validate = (currentPass) => {
        const validByr = currentPass.byr && parseInt(currentPass.byr) >= 1920 && parseInt(currentPass.byr) <= 2002
        const validIyr = currentPass.iyr && parseInt(currentPass.iyr) >= 2010 && parseInt(currentPass.iyr) <= 2020
        const validEyr = currentPass.eyr && parseInt(currentPass.eyr) >= 2020 && parseInt(currentPass.eyr) <= 2030
        let validHgt = currentPass.hgt && /^[0-9]{2,3}((in)|(cm))$/.test(currentPass.hgt)
        if (validHgt && currentPass.hgt.includes('in')) {
            const nb = parseInt(currentPass.hgt.split('in')[0])
            validHgt = nb >= 59 && nb <= 76
        }
        if (validHgt && currentPass.hgt.includes('cm')) {
            const nb = parseInt(currentPass.hgt.split('cm')[0])
            validHgt = nb >= 150 && nb <= 193
        }
        const validHcl = currentPass.hcl && /^(#[a-f0-9]{6})$/.test(currentPass.hcl)
        const validEcl = currentPass.ecl && /^((amb)|(blu)|(brn)|(gry)|(grn)|(hzl)|(oth))$/.test(currentPass.ecl)
        const calidPid = currentPass.pid && /^([0-9]{9})$/.test(currentPass.pid)
        return validByr && validIyr && validEyr && validHgt && validHcl && validEcl && calidPid
    }
    entries.forEach(entry => {
        if (entry.length > 0) {
            const infos = entry.split(' ')
