function solution1(input) {
    ...
    entries.forEach(entry => {
        ...
        splitTmp.forEach(tmp => {
            ...
            if (splitedBag[1] + ' ' + splitedBag[2]  === 'shiny gold') { 
                shinyGoldBags.push(index)
                verified[index] = true
            }
        })
    })
    while (shinyGoldBags.length > 0) {
        Object.keys(bags).forEach(bag => {
            if (bags[bag].find(tmp => tmp.name === shinyGoldBags[0]) && !verified[bag]) {
                shinyGoldBags.push(bag)
                verified[bag] = true
            }
        })
        shinyGoldBags.shift()
